    ...
    >
      ...
      <div className="flex flex-col border-t border-ink-900">
        {processStages.map((stage) => (
          <div
            key={stage.id}
            className="group relative isolate grid grid-cols-[1.4fr_1fr_140px] items-center gap-5 overflow-hidden border-b border-ink-900/20 py-[clamp(1.4rem,2.5vw,2.2rem)] max-lg:grid-cols-[46px_1fr_56px] max-sm:grid-cols-[40px_1fr_44px] max-sm:gap-3s"
          >
            ...
            <span className="font-mono text-xs uppercase tracking-kicker text-ink-700 max-lg:text-[0.7rem] hidden">
              {stage.id} 
            </span>
            <span className="font-display text-display-xl font-normal text-ink-900 max-lg:text-[clamp(1.5rem,5vw,2rem)] max-sm:text-[1.35rem]">
              {stage.name}
            </span>
